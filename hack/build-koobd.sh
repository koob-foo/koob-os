...
# This script compiles the koobd daemon (PID 1). It acts as the core init
# system and process supervisor for koob OS.
# Requires: go.
...
# Idempotency check
if [ -f "$DEST_BIN/koobd" ]; then
  # In CI, we trust the existence of the binary if the cache hit
  if [ -n "$GITHUB_ACTIONS" ]; then
    MSG="koobd restored from cache."
    printf "%b[ OK ]%b %s\n" "${OK_COLOR}" "${NC}" "$MSG"
    exit 0
  fi

  MSG="Skipping building koobd; binary already present in rootfs."
  printf "%b[ OK ]%b %s\n" "${OK_COLOR}" "${NC}" "$MSG"
  exit 0
fi

MSG="Compiling koobd daemon (PID 1)..."
printf "%b[ INFO ]%b %s\n" "${INFO_COLOR}" "${NC}" "$MSG"
...
CGO_ENABLED=0 go build -ldflags="$LDFLAGS" -o "$DEST_BIN/koobd" ./cmd/koobd

MSG="koobd binary installed to $DEST_BIN"
